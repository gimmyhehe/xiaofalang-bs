<template>
  <div class="app-levelbar-main clearfix">
    ...
    <el-button v-if="canGoBack" class="goback" type="text" icon="iconfont icon-back" @click="goBack" >返回上一级</el-button>
    <el-breadcrumb class="app-breadcrumb" separator="/">
      <transition-group name="breadcrumb">
        <el-breadcrumb-item v-for="(item,index)  in levelList" :key="item.path" v-if='item.name'>
          <router-link v-if='item.redirect==="noredirect"||index==levelList.length-1' to="" class="no-redirect">{{generateTitle(item)}}</router-link>
