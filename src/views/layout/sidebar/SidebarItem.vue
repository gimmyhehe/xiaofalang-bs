  ...
  <div class='menu-wrapper'>
    <template v-for="item in routes">
      ...
      <router-link
        ...
        <el-menu-item :index="(item.meta && item.meta.activeSideBar) || joinPath([prevPath,item.path,item.children[0].path])" class='submenu-title-noDropdown'>
          <span>{{generateTitle(item.children[0].name)}}</span>
        </el-menu-item>
      </router-link>

      <el-submenu :index="(item.meta && item.meta.activeSideBar) || item.name" v-if="!item.noDropdown&&!item.hidden" :key="'submenu'+item.name">
        <template slot="title">
          <!-- 菜单图标 -->
        ...
        <template v-for="child in item.children" v-if='!child.hidden'>
          ...
          <sidebar-item class='nest-menu' v-if='child.children&&child.children.length>0' :parentPath="item.path" :routes='[child]' :key="item.path+'/'+child.path"> </sidebar-item>

          <router-link v-else :to="item.path+'/'+child.path" :key="item.path+'/'+child.path">
            <el-menu-item :index="(item.meta && item.meta.activeSideBar) || item.path+'/'+child.path">
